        ...
        if st.button("Submit Guesses"):
            ...
            if not active_guesses:
                ...
                if len(st.session_state.active_players) == 1:
                    ...
                    # Add the "Did you know?" box after the game is over
                    with st.expander("Did you know?"):
                        st.write("When only two players remain in the game, there's a strategic rule in place: if one player guesses 0 and the other guesses 100, the player who guesses 100 wins. This rule introduces an interesting game theory element, as choosing 0 becomes a defensive move to avoid losing, but it also opens up a situation where guessing 100 can be a winning strategy. Essentially, it creates a dynamic similar to the rock-paper-scissors game, where the choices of 0, 1, and 100 interact with each other in a strategic manner. If you got this before opening this box, consider yourself quite smart!")
